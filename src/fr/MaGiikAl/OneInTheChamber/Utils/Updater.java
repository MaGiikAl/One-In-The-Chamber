...
public class Updater {
	...
	public Updater(Plugin plugin, int id, File file, UpdateType type, UpdateCallback callback, boolean announce) {
		...
		this.file = file;
		this.id = id;
		this.updateFolder = new File(this.plugin.getDataFolder(), "Update");
	...
	private void runUpdater() {
		if (this.url != null && (this.read() && this.versionCheck())) {
			...
			if ((this.versionLink != null) && (this.type != UpdateType.NO_DOWNLOAD)) {
				...
				this.saveFile(name);
				try {
					this.copyFileUsingStream(this.updateFolder.listFiles()[0], new File(this.plugin.getDataFolder().getParent(), this.file.getName()));
					delete(updateFolder);
				} catch (IOException e) {
					e.printStackTrace();
				}
